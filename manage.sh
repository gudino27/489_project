...
# Detect and set Docker commands (prefer /usr/bin/docker to avoid snap/old versions)
...
MIN_COMPOSE_VERSION="2.37.0"  # Relaxed from 2.39.1 - v2.37.x may work with COMPOSE_BAKE
...
setup_buildx_builder() {
    ...
    if ! $DOCKER_CMD buildx inspect default &>/dev/null; then
        log_info "Creating default BuildX builder..."
        $DOCKER_CMD buildx create --name default --driver docker-container --use --bootstrap 2>/dev/null || {
...
create_deployment_backup() {
    ...
    # Backup current $COMPOSE_CMD state
...
fast_deploy() {
    ...
    if fast_build "deploy"; then
        ...
        if [ -f "docker-compose.bake.yml" ]; then
            $COMPOSE_CMD -f docker-compose.bake.yml up -d
        else
            $COMPOSE_CMD up -d
        fi
...
deploy_zero_downtime() {
    ...
    if ! $COMPOSE_CMD -f docker-compose.deploy.yml build --no-cache; then
    ...
    if ! $COMPOSE_CMD -f docker-compose.deploy.yml up -d; then
...
case "$1" in
    ...
    check-versions|diagnose)
        ...
        $COMPOSE_CMD version 2>/dev/null
        echo ""

        echo "3. BuildX Plugin:"
        if $DOCKER_CMD buildx version 2>/dev/null; then
            echo ""
        else
        ...
        $DOCKER_CMD buildx ls 2>/dev/null || log_error "No builders found"
        echo ""

        echo "5. Docker Info:"
        $DOCKER_CMD info | grep -E "Server Version|Builder|BuildKit" || true
        echo ""
